...
S의 값들을 K로 나누어 리스트 L에 넣는다. <br>
곽철용의 점수 val보다 큰 조합의 개수를 최대화 해야한다. <br>
L을 내림차순 정렬 후 val보다 커지는 조합을 greedy하게 카운팅하면 최대의 조합 개수이다. <br> <br>
...
1. 첫 번째 수와 그 다음 수들을 차례대로 비교해 나가면서 그 값의 차이가 val보다 큰 지 확인한다. <br>
2. L[0]과 L[5]가 처음으로 만났을 때 val보다 커지므로 카운팅 한다. <br>
3. 그 다음 수 L[1]부터는 val보다 커지는 조합이 없으므로 반복문을 종료한다.
