...
### ordering
모델 개게의 리스트 출력 시 정렬하기 위해 사용하는 필드 명을 지정한다. 지정한 필드명을 기준으로 오름차순이 디폴트이고, 마이너스 접두사를 붙이면 내림차순으로 정렬한다. DB에 저장하는 순서를 의미하는 것은 아니고 DB에서 가져온 데이터를 출력할 때의 순서에 영향을 준다. 예를 들어 `Post` 클래스의 Meta 클래스에서는 `modify_dt`를 기준으로 내림차순으로 정렬한다. 
