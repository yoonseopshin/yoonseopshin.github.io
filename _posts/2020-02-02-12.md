...
<br>
이 때 $Find$ 연산의 각 연산마다 분할상환 시간은 $O(\alpha(n))$이며 여기서 $\alpha(n)$는 함수 $n = f(x) = A(x,x)$의 역함수이다. (*$A$는 극도로 빠르게 성장하는 아커만 함수*) 아커만 함수의 역함수이기 때문에 매우 느리게 증가한다. ($\alpha(2^65536) = 5$) 따라서 일반적인 입력에 대해 $O(\alpha(n)) \approx O(1)$라고 생각해도 무방하다.
