...
$c_n$은 2차원 정수 행렬에 $c1$, $c0$의 곱으로 나오며, $c1 = 6$, $c0 = 2$이므로 $c_n$은 항상 정수이다. 이 때, $b$는 1보다 작은 수이므로 $b^n$은 항상 1보다 작다. 즉, $c_n = a^n + 0.xxx$이므로 우리가 구하려는 $a_n$의 정수부분은 $c_n - 1$임이 자명하다.  
